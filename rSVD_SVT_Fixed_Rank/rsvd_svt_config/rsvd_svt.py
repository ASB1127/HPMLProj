"""
Configuration and runner for rSVD with Singular Value Thresholding (SVT) using fixed ranks.
This module uses the rSVDSVTAdam optimizer to perform low-rank adaptation
with periodic thresholding of singular values.
"""
import sys
import os
import numpy as np
...
def _get_rank_dir(rank, base_path="./graph"):
    ...
    return base_path + f"/r{rank}".replace(".", "_")
...
class RsvdsvtTrainer(Trainer):
    ...
    def __init__(self, rank, proj_interval, use_rgp, *args, **kwargs):
        self.rank = rank
...
class rsvd_svt_run():
    """Main runner class for rSVD experiments with fixed-rank SVT."""
    ...
    def __init__(self, num_train_epochs, rank, learning_rate, dataset="sst", 
        ...
        if self.dataset == 'imdb':
            self.dataset_config = {
                ...
                'eval_split': 'test',
                'remove_columns': ['text'],
                'save_dir': "./bert_imdb_checkpoints_" + f"r{rank}".replace(".", "_"),
            ...
            self.dataset_config = {
                ...
                'save_dir': "./bert_sst_checkpoints_" + f"r{rank}".replace(".", "_"),
                'model_name': 'distilbert-base-uncased',
                'max_length': 128,
                'batch_size': 32,
                'eval_batch_size': 64,
